...
static irqreturn_t hall_interrupt(int irq, void *data)
{
	...
	if (hall_gpio == hall_info->hall_switch_state)
		...
		pr_err("Change in hall sensor detected\n");

	input_report_switch(hall_info->ipdev, SW_LID, hall_gpio ? 0 : 1);
...
/*input system config*/
	...
	hall_info->ipdev->name = "hall_sensor";
        hall_info->ipdev->phys = "/dev/input/hall_dev";
	___set_bit(EV_SW, hall_info->ipdev->evbit);
